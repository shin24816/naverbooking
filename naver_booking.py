...
async def try_click_available_slot(page, test: bool = False) -> bool:
    """예약하기 링크를 클릭. test=True면 두상 교정, False면 영유아 검진(토요일)."""
    keyword = '두상' if test else '토요일'
    selector = await page.evaluate(f"""
        () => {{
            const links = Array.from(document.querySelectorAll('a[class*="link_desc_box"]'));
            if (!links.length) return null;
            const target = links.find(l => l.innerText.includes('{keyword}'));
            if (target) return links.indexOf(target);
            return 0;
        }}
    """)
    if selector is None:
...
async def select_date_and_time(page, test: bool = False, log: bool = False) -> bool:
    """달력에서 날짜/시간 선택 → (test면 두상 교정 체크박스) → 다음단계 클릭.
    test=False: 가장 빠른 토요일 / test=True: 가장 빠른 날짜"""
    ...
    for month_try in range(6):
        if test:
            ...
            # 실전: 가장 빠른 토요일 (7번째 열, index 6)
            date_result = await page.evaluate("""
                () => {
                    const dates = Array.from(document.querySelectorAll('[class*="calendar_date"]'))
                        .filter(el => el.className.trim() === 'calendar_date');
                    for (const date of dates) {
                        const td = date.closest('td');
                        if (!td) continue;
                        const cells = Array.from(td.parentElement.querySelectorAll('td'));
                        if (cells.indexOf(td) === 6) {
                            date.click();
                            return { ok: true, date: date.innerText.trim() };
                        }
                    }
                    return { ok: false };
                }
        ...
        if date_result['ok']:
            break
        # 이번 달 날짜 현황 로그 (테스트 모드만)
        if log:
            month_info = await page.evaluate("""
                () => {
                    const titleEl = document.querySelector('[class*="calendar_title"]');
                    const month = titleEl ? titleEl.innerText.trim() : '?';
                    const all = Array.from(document.querySelectorAll('[class*="calendar_date"]'));
                    const available = all
                        .filter(el => el.className.trim() === 'calendar_date')
                        .map(el => el.innerText.trim());
                    const unavailable = all
                        .filter(el => el.className.trim() !== 'calendar_date' && el.innerText.trim() !== '')
                        .map(el => {
                            const td = el.closest('td');
                            const col = td ? Array.from(td.parentElement.querySelectorAll('td')).indexOf(td) : -1;
                            return { date: el.innerText.trim(), cls: el.className.trim().replace('calendar_date', '').trim(), col };
                        });
                    return { month, available, unavailable };
                }
            """)
            unavail_items = [d for d in month_info['unavailable'] if d['col'] == 6] if not test else month_info['unavailable']
            unavail_str = ', '.join(f"{d['date']}일({d['cls']})" for d in unavail_items[:10])
            print(f"[달력] {month_info['month']}: 예약가능 {len(month_info['available'])}일 / 불가 {len(unavail_items)}일"
                  + (f" ({unavail_str})" if unavail_str else ""))
        # 이번 달에 가능한 날짜 없음 → '>' 버튼 클릭
        next_clicked = await page.evaluate("""
            () => {
                const btn = document.querySelector('[data-click-code="calendar.nextmonth"]');
    ...
    await asyncio.sleep(0.5)
    if log:
        all_time_slots = await page.evaluate("""
            () => Array.from(document.querySelectorAll(
                'button[class*="btn_time"], a[class*="btn_time"], li[class*="btn_time"]'
            )).map(el => ({ text: el.innerText.trim(), cls: el.className.trim() }))
        """)
        available_times = [t['text'] for t in all_time_slots if t['cls'] == 'btn_time']
        unavailable_times = [f"{t['text']}({t['cls'].replace('btn_time', '').strip()})" for t in all_time_slots if t['cls'] != 'btn_time']
        print(f"[시간] 예약가능: {', '.join(available_times) if available_times else '없음'}")
        if unavailable_times:
            print(f"[시간] 예약불가: {', '.join(unavailable_times[:15])}")
...
async def main(test: bool = False, test_infant: bool = False, target_time: datetime = None):
    try:
    ...
    if test:
        print("=" * 40)
        print("  테스트 모드 (두상 교정): 대기 없이 즉시 시작")
        print("=" * 40)
    elif test_infant:
        print("=" * 40)
        print("  테스트 모드 (영유아 검진): 5초 후 즉시 시작")
        print("=" * 40)
    elif target_time:
        print("=" * 40)
        print(f"  시간 지정 테스트: {target_time.strftime('%H:%M:%S')} KST에 예약 시작")
        print("=" * 40)
    ...
    async with async_playwright() as p:
        browser = await p.chromium.launch_persistent_context(
            user_data_dir=r"C:\Users\shin2\Documents\My Projects\naver_booking\naver_chrome_profile",
            headless=False,
            no_viewport=True,
        )
        ...
        if test_infant:
            ...
            for i in range(5, 0, -1):
                print(f"  로그인 확인 후 {i}초 뒤 시작...", end="\r")
                await asyncio.sleep(1)
            print("\n[시작] 영유아 검진 테스트 시작!")
        elif target_time:
            for i in range(5, 0, -1):
                print(f"  로그인 확인 후 {i}초 뒤 대기 시작...", end="\r")
                await asyncio.sleep(1)
            print(f"\n[대기] {target_time.strftime('%H:%M:%S')} KST까지 대기 중...")
        ...
        # test_infant는 실전 로직(토요일) 사용, test 플래그만 False로
        use_test_mode = test  # 두상 교정 여부
        ...
        while True:
            ...
            clicked = await try_click_available_slot(page, test=use_test_mode)
            if clicked:
                await asyncio.sleep(1)
                date_ok = await select_date_and_time(page, test=use_test_mode, log=test or test_infant or bool(target_time))
                if date_ok:
            ...
            if test or test_infant or target_time:
...
if __name__ == "__main__":
    ...
    TEST_INFANT_MODE = "--test-infant" in sys.argv

    TARGET_TIME_CUSTOM = None
    if "--test-time" in sys.argv:
        ...
        if idx + 1 < len(sys.argv):
            ...
            try:
                ...
                TARGET_TIME_CUSTOM = today.replace(hour=h, minute=m, second=s, microsecond=0)
            except (ValueError, AttributeError):
                print(f"[오류] 시간 형식이 잘못되었습니다: '{time_str}'")
                print("       올바른 형식: --test-time HH:MM:SS  (예: --test-time 09:00:30)")
                sys.exit(1)
        else:
            print("[오류] --test-time 뒤에 시간을 입력하세요  (예: --test-time 09:00:30)")
            sys.exit(1)

    asyncio.run(main(test=TEST_MODE, test_infant=TEST_INFANT_MODE, target_time=TARGET_TIME_CUSTOM))
